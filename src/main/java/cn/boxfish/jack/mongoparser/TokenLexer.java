package cn.boxfish.jack.mongoparser;

import org.slf4j.Logger;
import org.slf4j.LoggerFactory;

import java.io.BufferedReader;
import java.io.InputStreamReader;
import java.util.Arrays;
import java.util.regex.Matcher;
import java.util.regex.Pattern;

/**
 * Created by lvtu on 2017/5/16.
 */
public class TokenLexer implements SelectTokens{
    private final static Logger logger = LoggerFactory.getLogger(TokenLexer.class);

    private static final Pattern whiteSpacePattern = Pattern.compile("^\\s");
    private static final Pattern identifierPattern = Pattern.compile("^(\\w|-)+(_|-|[a-zA-Z])+(\\w|-)+");
    private static final Pattern numPattern = Pattern.compile("^\\d+");

    private static final Pattern keywordsPattern = Pattern.compile("^(AND|AS|BETWEEN|EXISTS|FALSE|FROM|IN|IS|LIKE|NOT|NULL|OR|SELECT|'|TRUE|UNKNOWN|WHERE|!|\\(|\\)|\\*|,|<|=|>)");

    public enum KeyWords{
        KEY_AND("AND", AND),
        KEY_AS("AS", AS),
        KEY_BETWEEN("BETWEEN", BETWEEN),
        KEY_EXISTS("EXISTS", EXISTS),
        KEY_FALSE("FALSE", FALSE),
        KEY_FROM("FROM", FROM),
        KEY_IN("IN", IN),
        KEY_IS("IS", IS),
        KEY_LIKE("LIKE", LIKE),
        KEY_NOT("NOT", NOT),
        KEY_NULL("NULL", NULL),
        KEY_OR("OR", OR),
        KEY_SELECT("SELECT", SELECT),
        KEY_SINGLEQUOTE("'", SINGLEQUOTE),
        KEY_TRUE("TRUE", TRUE),
        KEY_UNKNOWN("UNKNOWN", UNKNOWN),
        KEY_WHERE("WHERE", WHERE),
        KEY_EXCLAMATION_MARK("!", '!'),
        KEY_LEFT_PARENTHESIS("(", '('),
        KEY_RIGHT_PARENTHESIS(")", ')'),
        KEY_ASTERISK("*", '*'),
        KEY_COMMA(",", ','),
        KEY_LT("<", '<'),
        KEY_EQ("=", '='),
        KEY_GT(">", '>');

        int token;
        String value;
        KeyWords(String value,int token){
            this.value = value;
            this.token = token;
        }

        public static KeyWords get(String value){
            return Arrays.stream(values()).filter(item->item.value.equals(value)).findFirst().orElseThrow(RuntimeException::new);
        }

        public String getValue() {
            return value;
        }

        public int getToken() {
            return token;
        }
    }


    private BufferedReader reader = new BufferedReader(new InputStreamReader(System.in));

    private String command;

    public void readCommand(String sql) {
        command = sql;
    }

    private int token;
    private Object yylval;

    /**
     * Read the next token and return the
     * corresponding integer code.
     */
    public int nextToken() {
        logger.debug("scan tokens: {}", command);

        Matcher keywordsMatcher = keywordsPattern.matcher(command);
        if(keywordsMatcher.find()){
            yylval = keywordsMatcher.group();
            command = command.substring(keywordsMatcher.end());
            return token = KeyWords.get((String)yylval).getToken();
        }

        Matcher identifierMatcher = identifierPattern.matcher(command);
        if (identifierMatcher.find()) {
            yylval = identifierMatcher.group();
            command = command.substring(identifierMatcher.end());
            return token = IDENTIFIER;
        }

        Matcher numMatcher = numPattern.matcher(command);
        if (numMatcher.find()) {
            yylval = Integer.valueOf(numMatcher.group());
            command = command.substring(numMatcher.end());
            return token = NUMBER;
        }

        Matcher whitespaceMatcher = whiteSpacePattern.matcher(command);
        if (whitespaceMatcher.find()) {
            command = command.substring(whitespaceMatcher.end());
            return nextToken();
        }

        return ENDINPUT;
    }

    /**
     * Return the token code for the current lexeme.
     */
    int getToken() {
        return token;
    }

    /**
     * Return the semantic value for the current lexeme.
     */
    Object getSemantic() {
        return yylval;
    }
}
